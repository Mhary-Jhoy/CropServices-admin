...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    document
      .getElementById("btn-dashboard-tools")
      .addEventListener("click", () => btnTools.click());
    ...
    btnInsertTools.addEventListener("click", async () => {
      if (
        ...
        try {
          ...
          await set(child(toolsRef, `${toolsID}`), toolData).then(async () => {
          ...
          for (const toolID in toolsData) {
            ...
            btnDelete.addEventListener("click", async () => {
              try {
                ...
                console.error(`Error deleting tool: ${data.name}`, error);
              }
            });
            ...
            btnEdit.addEventListener("click", async () => {
              if (tdOwner.contentEditable === "false") {
                ...
                try {
                  ...
                  console.error(`Error updating tool: ${data.name}`, error);
                }
              }
            });
          }
